  ...
  if (mode === "normal") {
    for (let i = 0; i < times; i++) {
      ...
      for (const i of crateItems) {
        if (items[i]) {
          if (items[i].rarity === 6) {
            ...
            for (let x = 0; x < 2; x++) {
              if (items[i].role == "collectable" || items[i].role === "flower" || items[i].role === "cat") {
                const chance = Math.floor(Math.random() * 3);
      ...
      if (chosen.includes("money:") || chosen.includes("xp:")) {
        ...
        found.set(`${type}:${amount}`, found.has(`${type}:${amount}`) ? found.get(`${type}:${amount}`) + 1 : 1);
      } else {
        let amount = 1;
    ...
    for (let i = 0; i < times; i++) {
      ...
      for (const itemFilter of item.items) {
        ...
        if (itemFilter.startsWith("id:")) {
          ...
          crateItems.push(...Object.keys(items).filter((i) => items[i].role === itemFilter.split(":")[1]));
        } else {
          crateItems.push(itemFilter);
          continue;
        }
